...
export function SectionHero({ eyebrow, title, description, actions, aside, className = '' }) {
  return (
    <section className={`panel-glass techsim-hero-card relative overflow-hidden rounded-[30px] p-6 sm:p-7 ${className}`}>
      ...
      <div className="relative z-[1] grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
        <div>
          {eyebrow && <div className="eyebrow mb-3">{eyebrow}</div>}
          <h1 className="font-display text-2xl font-semibold tracking-tight text-[var(--text)] sm:text-3xl">{title}</h1>
          {description && <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--text-soft)] sm:text-[15px]">{description}</p>}
...
export function MetricCard({ icon, label, value, hint, color = 'var(--accent)', compact = false }) {
  return (
    <div
      className={`techsim-stat-card rounded-[24px] border p-4 ${compact ? 'min-h-[120px]' : 'min-h-[138px]'}`}
      style={{ borderColor: `color-mix(in srgb, ${color} 26%, var(--border))` }}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl border" style={{ color, borderColor: `color-mix(in srgb, ${color} 35%, transparent)`, background: `color-mix(in srgb, ${color} 15%, transparent)` }}>
          <AppIcon name={icon} className="h-5 w-5" />
        </div>
        <span className="techsim-kicker">live</span>
      </div>
      <div className="mt-5 text-2xl font-semibold tracking-tight text-[var(--text)]">{value}</div>
      <div className="mt-1 text-sm font-medium text-[var(--text-soft)]">{label}</div>
      {hint ? <div className="mt-3 text-xs leading-6 text-[var(--text-dim)]">{hint}</div> : null}
    </div>
  );
}
...
export function EmptyState({ icon = 'info', title, description, action }) {
  return (
    <div className="techsim-empty rounded-[24px] border border-dashed border-[var(--border)] bg-[color:var(--surface)]/70 p-8 text-center">
      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-white/8 bg-white/[0.03] text-[var(--text-dim)]">
...
export function ModuleBadge({ color, icon, label, meta }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.03] px-3.5 py-3">
