  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-hidden" style={{ fontFamily: 'var(--font-body)' }}>
      <SectionHero
        ...
        actions={(
          <>
            <button type="button" onClick={() => setActiveTab('modules')} className="ts-btn ts-btn-primary rounded-full px-4 py-2.5 text-[10px]">ABRIR MÓDULOS</button>
            <button type="button" onClick={() => setActiveTab('projects')} className="ts-btn ts-btn-ghost rounded-full px-4 py-2.5 text-[10px]">VER PROJETOS</button>
            {isAdmin && (
              <button type="button" onClick={onAdmin} className="ts-btn rounded-full border border-rose-400/30 bg-rose-500/10 px-4 py-2.5 text-[10px] text-rose-200 transition hover:border-rose-400/50 hover:bg-rose-500/16">
                PAINEL ADMIN
              </button>
            )}
          </>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        <aside className="panel-glass editor-scroll flex min-h-0 flex-col gap-3 overflow-y-auto rounded-[24px] p-3.5">
            ...
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex items-center gap-2.5 overflow-hidden rounded-2xl border px-3 py-2.5 text-left text-sm font-medium transition-all duration-200 ${active ? 'border-violet-400/45 bg-violet-500/16 text-violet-100 shadow-[0_10px_26px_-12px_rgba(139,92,246,0.55)]' : 'border-white/8 bg-white/[0.02] text-[var(--text-soft)] hover:border-white/14 hover:bg-white/[0.04] hover:text-[var(--text)]'}`}
                >
                  {active && <span className="absolute inset-y-1.5 left-0 w-[3px] rounded-full bg-violet-400" />}
                  <span className={`flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${active ? 'bg-violet-500/20 text-violet-200' : 'bg-white/[0.03] text-[var(--text-dim)] group-hover:text-[var(--text-soft)]'}`}>
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-3.5">
            ...
            <div className="space-y-1.5">
              {latestProjects.length ? latestProjects.map(project => {
                ...
                return (
                  <button key={project.id} type="button" onClick={() => onOpenModule(project.moduleId, project.id)} className="group flex w-full items-center gap-2.5 rounded-xl border border-white/8 bg-black/15 px-2.5 py-2.5 text-left transition-all duration-200 hover:border-white/16 hover:bg-black/25 hover:translate-x-0.5">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-transform duration-200 group-hover:scale-105" style={{ background: hexToRgba(module?.color || '#8b5cf6', 0.16), color: module?.color || '#c4b5fd' }}>
          ...
          <button onClick={onLogout} className="ts-btn ts-btn-ghost mt-auto flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[10px]">
