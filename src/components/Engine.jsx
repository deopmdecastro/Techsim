...
export function Engine({ modId, modColor, lib, userName, modulePresets = [], savedProjects = [], onSaveProject, onLoadProject, initialProject, initialProjectKey = 'default' }) {
  ...
  return (
    <div style={{ display: 'flex', flex: 1, overflow: 'hidden', height: '100%', fontFamily: "'Courier New','Consolas',monospace" }}>
      <div style={{ width: 180, background: '#040d18', borderRight: '1px solid #1e293b', display: 'flex', flexDirection: 'column', padding: '10px 8px', gap: 8, overflowY: 'auto', flexShrink: 0 }}>
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>TOOLS</div>
        {[{ t: 'select', lbl: 'Selecionar', sym: TOOL_GLYPHS.select, color: '#fb7185' }, { t: 'wire', lbl: 'Conectar', sym: TOOL_GLYPHS.wire, color: '#94a3b8' }, { t: 'delete', lbl: 'Excluir', sym: TOOL_GLYPHS.delete, color: '#f87171' }].map(button => <button key={button.t} onClick={() => setTool(button.t)} style={{ background: tool === button.t ? `${button.color}16` : '#050e1a', border: `1px solid ${tool === button.t ? button.color : '#1e293b'}`, color: tool === button.t ? button.color : '#475569', borderRadius: 10, padding: '8px 5px', cursor: 'pointer' }}>{button.sym} {button.lbl}</button>)}
        <div style={{ display: 'grid', gap: 6 }}>
          <button onClick={groupSelection} style={{ background: '#071020', border: '1px solid #1e293b', color: '#a78bfa', borderRadius: 8, padding: '6px 8px' }}>⧉ Agrupar</button>
          <button onClick={ungroupSelection} style={{ background: '#071020', border: '1px solid #1e293b', color: '#94a3b8', borderRadius: 8, padding: '6px 8px' }}>↯ Desagrupar</button>
        </div>
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>PÁGINAS</div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{pages.map(page => <button key={page.id} onClick={() => switchPage(page.id)} style={{ background: page.id === activePageId ? `${modColor}18` : '#071020', border: `1px solid ${page.id === activePageId ? modColor : '#1e293b'}`, color: page.id === activePageId ? modColor : '#64748b', borderRadius: 999, padding: '4px 8px', fontSize: 7 }}>{page.name}</button>)}<button onClick={addPage} style={{ background: '#071020', border: '1px dashed #334155', color: '#94a3b8', borderRadius: 999, padding: '4px 8px', fontSize: 7 }}>+ Página</button></div>
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>CAMADAS</div>
        <button onClick={addLayer} style={{ background: '#071020', border: '1px dashed #334155', color: '#94a3b8', borderRadius: 8, padding: '6px 8px' }}>+ Camada</button>
        {(activePage?.layers || []).map(layer => <div key={layer.id} style={{ background: layer.id === currentLayerId ? `${modColor}12` : '#050e1a', border: `1px solid ${layer.id === currentLayerId ? modColor : '#1e293b'}`, borderRadius: 8, padding: 6 }}><div style={{ display: 'flex', gap: 4, alignItems: 'center' }}><button onClick={() => updateActivePageMeta(page => ({ ...page, currentLayerId: layer.id }))} style={{ flex: 1, background: 'transparent', border: 'none', color: layer.id === currentLayerId ? modColor : '#cbd5e1', textAlign: 'left' }}>{layer.name}</button><button onClick={() => toggleLayer(layer.id, 'visible')} style={{ background: 'transparent', border: 'none', color: '#94a3b8' }}>{layer.visible === false ? '🙈' : '👁'}</button><button onClick={() => toggleLayer(layer.id, 'locked')} style={{ background: 'transparent', border: 'none', color: layer.locked ? '#f87171' : '#22c55e' }}>{layer.locked ? '🔒' : '🔓'}</button></div><button onClick={() => moveSelectionToLayer(layer.id)} style={{ width: '100%', marginTop: 4, background: '#071020', border: '1px solid #1e293b', color: '#64748b', borderRadius: 6, padding: '3px 6px', fontSize: 7 }}>Mover seleção</button></div>)}
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>GRUPOS</div>
        {(activePage?.groups || []).map(group => <button key={group.id} onClick={() => setSelectionState(group.memberIds, group.memberIds[0])} style={{ background: '#071020', border: '1px solid #1e293b', color: '#a78bfa', borderRadius: 8, padding: '6px 8px', textAlign: 'left' }}>{group.name}<div style={{ color: '#64748b', fontSize: 6 }}>{group.memberIds.length} itens</div></button>)}
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>PRESETS</div>
        {modulePresets.map(preset => <button key={preset.id} onClick={() => applyProjectState(preset.project, `Preset carregado: ${preset.title}`)} style={{ background: '#071020', border: `1px solid ${hexToRgba(modColor, 0.22)}`, borderRadius: 10, padding: '8px 8px', textAlign: 'left' }}><div style={{ fontSize: 8, color: modColor, fontWeight: 700 }}>{preset.title}</div><div style={{ fontSize: 6.5, color: '#64748b' }}>{preset.description}</div></button>)}
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>PROJETOS</div>
        {savedProjects.slice(0, 6).map(project => <button key={project.id} onClick={() => loadSavedProject(project.id)} disabled={loadingProjectId === project.id} style={{ background: '#050e1a', border: '1px solid #1e293b', borderRadius: 10, padding: '8px 8px', textAlign: 'left' }}><div style={{ fontSize: 7.8, color: modColor, fontWeight: 700 }}>{project.name}</div><div style={{ fontSize: 6.2, color: '#475569' }}>{project.summary}</div><div style={{ fontSize: 5.8, color: '#334155' }}>{prettyDate(project.updatedAt)}</div></button>)}
        <div style={{ fontSize: 7, color: '#1e3a5f', textAlign: 'center', letterSpacing: 2 }}>COMPONENTES</div>
        <input value={paletteFilter} onChange={event => setPaletteFilter(event.target.value)} placeholder="buscar" style={{ background: '#071020', border: '1px solid #1e293b', color: '#cbd5e1', padding: '6px 8px', borderRadius: 8, fontSize: 10 }} />
        {filteredLib.map(item => <button key={item.t} draggable onDragStart={event => event.dataTransfer.setData('text/plain', item.t)} onClick={() => setTool(item.t)} style={{ background: tool === item.t ? `${item.col}18` : '#050e1a', border: `1px solid ${tool === item.t ? item.col : '#1e293b'}`, color: tool === item.t ? item.col : '#94a3b8', borderRadius: 10, padding: '8px 6px' }}>{item.sym} {item.lbl}</button>)}
      </div>
